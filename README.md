# 🦅 Caio Henrique Santos Carvalho 🦅

🎓 Estudante de **Ciência da Computação** na Universidade Presbiteriana Mackenzie  
💻 Interessado em **Desenvolvimento de Software, Redes e Cybersecurity**  
🔐 Membro da **Liga Macksec - Cybersecurity**  
📍 Osasco, São Paulo - Brasil

---

## 🧑‍💻 Sobre mim

Sou estudante de Ciência da Computação, atualmente no 5º semestre, 
buscando oportunidades para aplicar na prática os conhecimentos adquiridos
na universidade e continuar desenvolvendo minhas habilidades.

Tenho interesse principalmente em **desenvolvimento de software,
redes de computadores e cybersecurity**, buscando constantemente aprender
novas tecnologias e aprimorar meus conhecimentos.

> "Aprender, construir e evoluir."

---

## 🚀 Tecnologias & Conhecimentos

### 💻 Linguagens de Programação

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=black)
![Haskell](https://img.shields.io/badge/Haskell-5D4F85?style=for-the-badge&logo=haskell&logoColor=white)
![Prolog](https://img.shields.io/badge/Prolog-E61B23?style=for-the-badge&logoColor=white)

### 🌐 Web & Banco de Dados

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![Oracle](https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white)

### 🔐 Cybersecurity & Redes

![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Wireshark](https://img.shields.io/badge/Wireshark-1679A7?style=for-the-badge&logo=wireshark&logoColor=white)

**Ferramentas:**

`Nmap` · `Netcat` · `Burp Suite` · `Gobuster` · `Wireshark`

---

## 📚 Formação

🎓 **Bacharelado em Ciência da Computação**  
Universidade Presbiteriana Mackenzie — São Paulo, SP

**5º semestre**  
Previsão de conclusão: **Julho de 2028**

🔐 **Liga Macksec - Cybersecurity**

---

## 📜 Cursos & Certificações

🐍 **PCAP: Programming Essentials in Python**  
Cisco Networking Academy

🌐 **5G - Conceitos Básicos de Redes**  
Cisco Networking Academy

---

## 🛠️ Áreas de Interesse

```text
💻 Desenvolvimento de Software
🔐 Cybersecurity
🌐 Redes de Computadores
🐍 Python
☕ Java
🖥️ Linux
📊 Banco de Dados
🧠 Estruturas de Dados & Algoritmos
